	...
	static class LinkChecker {
		public static bool IsPossibleConnection(ItemQualityPair item, ReadOnlyBaseNode supplier, ReadOnlyBaseNode consumer) {
			if (!supplier.Outputs.Contains(item) || !consumer.Inputs.Contains(item))
				return false;
			if (!(item.Item is IFluid fluid) || !fluid.IsTemperatureDependent)
				return true;
		...
		public static FRange GetTemperatureRange(IFluid fluid, ReadOnlyBaseNode node, LinkType direction, bool includeSelf) {
			...
			HashSet<ReadOnlyBaseNode> visitedNodes = [];
			void Internal_GetMinMaxTempForNode(ReadOnlyBaseNode cNode) {
				...
				if (cNode is ReadOnlyRecipeNode rorn && (cNode != node || includeSelf)) //RecipeNode
				{
					var recipe = rorn.BaseRecipe.Recipe;
					if (direction == LinkType.Input && recipe.IngredientSet.ContainsKey(fluid)) //have to check for ingredient inclusion due to fuel/fuel-remains
					{
						...
						gotOne = true;
					} else if (direction == LinkType.Input && (((ReadOnlyRecipeNode)cNode).SelectedAssembler.Assembler?.IsTemperatureFluidBurner ?? false)) //special case for fluid burner
					  {
						minTemp = Math.Max(minTemp, ((ReadOnlyRecipeNode)cNode).SelectedAssembler.Assembler?.FluidFuelTemperatureRange.Min ?? minTemp);
						maxTemp = Math.Min(maxTemp, ((ReadOnlyRecipeNode)cNode).SelectedAssembler.Assembler?.FluidFuelTemperatureRange.Max ?? maxTemp);
						gotOne = true;
